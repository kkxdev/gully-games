...
export function completeRound(match: Match, outcome: RoundOutcome): Match {
  ...
  return {
    scores,
    round: match.round + (scores[outcome] < 3 ? 1 : 0),
    winner: scores[outcome] >= 3 ? outcome : null,
  };
}
...
export function calculateFlick(origin: Vector, pointer: Vector, maxDrag = 130) {
  const dx = origin.x - pointer.x,
    dy = origin.y - pointer.y;
  const length = Math.hypot(dx, dy);
  const power = Math.min(1, length / maxDrag);
  return { x: length ? dx / length : 0, y: length ? dy / length : 0, power };
}
export function cpuFlick(
  cpu: Vector,
  player: Vector,
  random: () => number = Math.random,
) {
  const angle =
    Math.atan2(player.y - cpu.y, player.x - cpu.x) + (random() - 0.5) * 0.22;
  return {
    x: Math.cos(angle),
    y: Math.sin(angle),
    power: Math.min(
      0.85,
      0.4 +
        Math.hypot(player.x - cpu.x, player.y - cpu.y) / 850 +
        random() * 0.12,
    ),
  };
}
